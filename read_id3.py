...
with open(filename, 'rb') as mp3_file:
    ...
    if header[:5] == b'ID3\x03\x00':
        ...
        while True:
            ...
            frameHeader = mp3_file.read(10)
            frame_id = frameHeader[:4]

            if frame_id in frame_types:
                print(f'Found frame type: {frame_id}')

                # We need the frame size.
                frameSize = int.from_bytes(frameHeader[4:8], 'big')
                ...
                if frame_id.startswith(b'T'):  # a text field
                    ...
                    text = mp3_file.read(frameSize - 1).decode(encoding)
                    ...
                    description_and_url = mp3_file.read(frameSize - 1)
                    ...
                    # Get the encoding Block_6byte.
                    encoding_byte = mp3_file.read(1)[0]
                    encoding = id3_field_encodings[encoding_byte]
                    print(f'APIC text encoding: {encoding}')

                    # Next we have a null-terminated string.
                    mimeType = read_c_string(mp3_file, 'iso-8859-1')
                    if mimeType == '': #check if there is no image
                        mimeType = 'image/'
                    print(f'Mime Type: {mimeType}')

                    # read 1 byte picture type
                    pictureType = int.from_bytes(mp3_file.read(1), 'big')
                    # and get its human name
                    apicPictureName = apic_picture_types[pictureType]
                    print(f'Found {apicPictureName} image')
                    ...
                    if mimeType.startswith('image/'):
                        imageDataStart = mp3_file.tell()
                        print(f'Image data starts at {imageDataStart}')
                        imageSize = frameSize - (imageDataStart - frame_data_start)
                        print(f'Image size = {imageSize}')
                        imageData = mp3_file.read(imageSize)

                        #create file name from the picture name
                        imageType = mimeType.split('/')[-1]

                        # get file name part only (without the path)
                        baseFileName = path.split(filename)[1]

                        # now we can remove the extension
                        baseFileName = path.splitext(baseFileName)[0]

                        pictureFileName = f'{baseFileName}_{apicPictureName}.{imageType}'
                        print(f'Writing image file {pictureFileName}')

                        with open(pictureFileName, 'wb') as outputFile:
                            outputFile.write(imageData)

                else:
                    # Found a frame that we're not going to process.
                    # Skip it by seeking forward `frame_size` bytes
                    mp3_file.seek(frameSize, SEEK_CUR)
